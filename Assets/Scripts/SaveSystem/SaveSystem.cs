...
public static class SaveSystem
{
    ...
    public static void Init()
    {
        ...
        {
            isInit = true;
            // Test if GetSaveObject Folder exists
            if (!Directory.Exists(SAVE_FOLDER))
            {
                // Create GetSaveObject Folder
    ...
    public static void Save(string fileName, string saveString, bool overwrite)
    {
        ...
        if (!overwrite)
        {
            // Make sure the GetSaveObject Number is unique so it doesnt overwrite a previous save file
            int saveNumber = 1;
